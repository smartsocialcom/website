document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    const { total_students, parents: parentsCount, custom_graphics, users_per_month_arr, school_buildings } = data.organization;
    ...
    const results = data.log.reduce((acc, { created_at, page_url, user, school_buildings_id }) => {
        const validPaths = [
    ...
    new Chart(document.getElementById("topSchoolBuildings"), {
      ...
      data: {
        ...
        datasets: [{
          data: topSchoolBuildings.map(item => (item.count / totalTopSchoolBuildings * 100).toFixed(2)),
